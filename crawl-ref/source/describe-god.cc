...
static void build_partial_god_ui(god_type which_god, shared_ptr<UIPopup>& popup, shared_ptr<UISwitcher>& desc_sw, shared_ptr<UISwitcher>& more_sw)
{
    ...
    auto title = make_shared<UIText>(trimmed_string(topline.to_colour_string()));
    title->set_margin_for_crt({0, 0, 0, 0});
    title->set_margin_for_sdl({0, 0, 0, 16});
    title_hbox->add_child(move(title));

    title_hbox->align_items = UI::CENTER;
    title_hbox->align_self = UI::CENTER;
    vbox->add_child(move(title_hbox));

    desc_sw = make_shared<UISwitcher>();
    more_sw = make_shared<UISwitcher>();
    desc_sw->current() = 0;
    more_sw->current() = 0;
...
# define MORE_PREFIX "[<w>!</w>/<w>^</w>" "]: "
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        auto text = make_shared<UIText>(trimmed_string(desc.to_colour_string()));
        text->wrap_text = true;
        scroller->set_child(text);
        desc_sw->add_child(move(scroller));

        more_sw->add_child(make_shared<UIText>(mores[i]));
    }
...
void describe_god(god_type which_god)
{
    ...
    shared_ptr<UIPopup> popup;
    shared_ptr<UISwitcher> desc_sw;
    shared_ptr<UISwitcher> more_sw;
...
bool describe_god_with_join(god_type which_god)
{
    ...
    for (int i = JOIN; i <= ABANDON; i++)
    {
        ...
        more_sw->add_child(make_shared<UIText>(prompt_fs.to_colour_string()));
